...
function TimetableGeneration({ data, updateData, nextStep, prevStep }) {
  ...
  // 헬퍼 함수: 교사별 현재 시수 계산
  const getCurrentTeacherHours = (schedule, teacherName, specificClassName = null) => {
    let hours = 0;
  ...
  // 헬퍼 함수: 학급별 과목 시수 계산
  const getClassSubjectHours = (schedule, className) => {
    const subjectHours = {};
  ...
  const convertClassNameToKey = (className) => {
  ...
  const generateTimetable = async () => {
    ...
    try {
      ...
      if (dailySubjectOnceConstraints.length > 0) {
        addLog('📋 적용되는 학급 일일 과목 1회 제한 제약조건:', 'info');
        dailySubjectOnceConstraints.forEach((constraint, index) => {
          const priority = (data.constraints?.must || []).some(c => c.id === constraint.id) ? '필수' : '선택';
          const subjectText = constraint.subject === 'all' ? '모든 수업에 해당' : constraint.subject;
          addLog(`  ${index + 1}. ${subjectText} (${priority} 조건)`, 'info');
        });
      }
      ...
      for (let grade = 1; grade <= grades; grade++) {
        ...
        for (let classNum = 1; classNum <= classesInGrade; classNum++) {
          ...
          if (data.classWeeklyHours && data.classWeeklyHours[className] === 0) {
            addLog(`🚫 ${className}: 0시간 설정으로 스케줄에서 완전 제외`, 'warning');
            continue;
          }
      ...
      // 0시간 설정으로 제외된 학급 수 계산
      const totalPossibleClasses = Array.from({length: grades}, (_, i) => classesPerGrade[i] || 0).reduce((sum, count) => sum + count, 0);
      const excludedClasses = totalPossibleClasses - classNames.length;
      
      addLog(`${classNames.length}개 학급의 시간표 구조를 생성했습니다.`, 'success');
      if (excludedClasses > 0) {
        addLog(`🚫 ${excludedClasses}개 학급이 0시간 설정으로 제외되었습니다.`, 'warning');
      }
      ...
      addLog('4단계: 교사 일일 학급 중복 금지 제약조건을 검증합니다.', 'info');
      processTeacherSameClassDailyLimit(schedule, addLog);
      setGenerationProgress(40);

      // 5단계: 교사별 시수 추적 초기화
      addLog('5단계: 교사별 시수를 추적합니다.', 'info');
      const teacherHours = {};
      const teachers = data.teachers || [];
      teachers.forEach(teacher => {
        ...
        // 각 학급별 현재 시수와 최대 시수 초기화
        classNames.forEach(className => {
          const classKey = convertClassNameToKey(className);
          const maxClassHours = (teacher.classWeeklyHours && teacher.classWeeklyHours[className]) ||
                               (teacher.weeklyHoursByGrade && teacher.weeklyHoursByGrade[classKey]) || 0;
      ...
      // 교사별 수업 불가 시간 설정 확인
      addLog('📅 교사별 수업 불가 시간 설정 확인:', 'info');
      teachers.forEach(teacher => {
        if (teacher.unavailable && Array.isArray(teacher.unavailable) && teacher.unavailable.length > 0) {
          const unavailableSlots = teacher.unavailable.map(([day, period]) => `${day}요일 ${period}교시`).join(', ');
          addLog(`  🚫 ${teacher.name}: ${unavailableSlots}`, 'warning');
        } else {
          addLog(`  ✅ ${teacher.name}: 수업 불가 시간 없음`, 'info');
        }
      });
      ...
      // 6단계: 과목별 배치 계획 수립
      addLog('6단계: 과목별 배치 계획을 수립합니다.', 'info');
      ...
      // 7단계: 과목 배치 실행
      addLog('7단계: 과목을 배치합니다.', 'info');
      
      let placedCount = 0;
      const maxAttempts = subjectPlacementPlan.length * 100; // 시도 횟수 증가
      let attempts = 0;
      ...
      while (subjectPlacementPlan.length > 0 && attempts < maxAttempts) {
        ...
        const teachersWithPriority = placement.availableTeachers.map(teacher => {
          ...
          // 1. 공동수업 제약조건이 있는 교사는 우선순위를 낮춤
          const coTeachingConstraints = (data.constraints?.must || []).filter(c =>
            c.type === 'specific_teacher_co_teaching' && c.mainTeacher === teacher.name
          );
          if (coTeachingConstraints.length > 0) {
            priority += 1000; // 공동수업 제약조건이 있는 교사는 우선순위를 낮춤
          }
        ...
        for (const teacherInfo of teachersWithPriority) {
          ...
          if (teacherHours[teacher.name]?.current >= teacherHours[teacher.name]?.max) {
            continue; // 시수 초과 교사는 건너뛰기
          }
          
          // 교사가 해당 학급을 담당하는지 재확인 (2단계 전략 적용)
          ...
          if (availableSlots.length > 0) {
            ...
            // 최종 안전 확인: 0시간 설정 학급 체크
            if (!canPlaceClassInSchedule(placement.className, addLog)) {
              continue;
            }
            ...
            if (!validateSlotPlacement(schedule, placement.className, selectedSlot.day, selectedSlot.period, teacher, placement.subject, addLog)) {
              addLog(`⚠️ 중복 검증 실패: ${placement.className} ${selectedSlot.day} ${selectedSlot.period}교시 ${placement.subject} 배치 건너뜀`, 'warning');
              continue;
            }
            ...
            if (placedCount % 50 === 0) {
              ...
              if (dailySubjectOnceConstraints.length > 0) {
                addLog(`제약조건 적용 확인: ${placement.className} ${selectedSlot.day}요일 ${selectedSlot.period}교시에 ${placement.subject} 배정 완료`, 'info');
              }
            }
            
            if (placedCount % 20 === 0) {
              addLog(`${placedCount}개 수업 배치 완료...`, 'info');
              setGenerationProgress(60 + (placedCount / (subjectPlacementPlan.length + placedCount)) * 30);
            }
            break;
          }
        }
        ...
        if (!placed) {
          // 재시도 로직
          placement.priority += 0.1; // 우선순위 낮춤
          if (placement.priority < 20) { // 최대 20번 재시도 (증가)
            subjectPlacementPlan.push(placement);
          } else {
            addLog(`경고: ${placement.className}의 ${placement.subject} 과목을 배치할 수 없습니다. (재시도 ${Math.floor(placement.priority * 10)}회)`, 'warning');
          }
        }
      }
                ...
                if (availableSubjects.length > 0) {
                  ...
                  availableSubjects.forEach(subject => {
                    const availableTeachers = teachers.filter(teacher => {
                      ...
                      if (!hasSubject || !hasCapacity) {
                        return false;
                      }
                      
                      // 2단계 전략: 명시적 금지가 없으면 허용
                      const classKey = convertClassNameToKey(className);
                      const isExplicitlyForbidden = (teacher.classWeeklyHours && teacher.classWeeklyHours[className] === 0) ||
                                                   (teacher.weeklyHoursByGrade && teacher.weeklyHoursByGrade[classKey] === 0);
                      ...
                      // 명시적 할당이 있는 경우에만 시수 제한 확인
                      const hasExplicitAssignment = (teacher.classWeeklyHours && teacher.classWeeklyHours[className] > 0) ||
                                                   (teacher.weeklyHoursByGrade && teacher.weeklyHoursByGrade[classKey] > 0);
                      
                      if (hasExplicitAssignment) {
                        const classHoursCheck = checkTeacherClassHoursLimit(teacher, className, schedule);
                        if (!classHoursCheck.allowed) {
                          return false; // 시수 제한 초과
                        }
                      }
                  ...
                  if (subjectTeacherPairs.length > 0) {
                    ...
                    // 최종 안전 확인: 0시간 설정 학급 체크
                    if (!canPlaceClassInSchedule(className, addLog)) {
                      return;
                    }
                    
                    // 🔒 일반 빈 슬롯 채우기 전 최종 중복 검증 (같은 교시 중복 방지)
                    if (!validateSlotPlacement(schedule, className, day, period, selectedPair.teacher, selectedPair.subject.name, addLog)) {
                      addLog(`⚠️ 일반 빈슬롯 중복 검증 실패: ${className} ${day} ${period}교시 ${selectedPair.subject.name} 배치 건너뜀`, 'warning');
                      return;
                    }
      ...
      // 9단계: 공동수업 제약조건 검증
      addLog('9단계: 공동수업 제약조건을 검증합니다.', 'info');
      validateCoTeachingConstraints(schedule, addLog);
      setGenerationProgress(95);

      // 10단계: 최종 검증 및 결과 표시
      addLog('10단계: 생성된 시간표를 검증하고 교사별 시수 균형을 확인합니다.', 'info');
      ...
      teachers.forEach(teacher => {
        ...
        if (teacher.unavailable && Array.isArray(teacher.unavailable)) {
          teacher.unavailable.forEach(([day, period]) => {
            Object.keys(schedule).forEach(className => {
              if (schedule[className][day] && schedule[className][day][period - 1]) {
                const slot = schedule[className][day][period - 1];
                if (slot && typeof slot === 'object' && slot.teachers && slot.teachers.includes(teacher.name)) {
                  unavailableViolations++;
                  unavailableDetails.push(`${className} ${day}요일 ${period}교시(${slot.subject})`);
                }
              }
            });
          });
        ...
        classNames.forEach(className => {
          const classKey = convertClassNameToKey(className);
          const maxHours = (teacher.classWeeklyHours && teacher.classWeeklyHours[className]) ||
                         (teacher.weeklyHoursByGrade && teacher.weeklyHoursByGrade[classKey]) || 0;
          if (maxHours > 0) {
            const currentHours = getCurrentTeacherHours(schedule, teacher.name, className);
            if (currentHours > maxHours) {
              classHoursViolations++;
              violations.push(`${className}: ${currentHours}/${maxHours}시간`);
            }
          }
        });
        
        if (unavailableViolations > 0 || classHoursViolations > 0) {
          addLog(`  ❌ ${teacher.name}: 수업불가시간 위반 ${unavailableViolations}건, 학급시수 위반 ${classHoursViolations}건`, 'error');
          if (unavailableViolations > 0 && unavailableDetails.length > 0) {
            addLog(`    🚫 수업불가시간 위반: ${unavailableDetails.join(', ')}`, 'error');
          }
          if (violations.length > 0) {
            addLog(`    ⚠️ 학급시수 위반: ${violations.join(', ')}`, 'warning');
          }
        } else {
          addLog(`  ✅ ${teacher.name}: 모든 제약조건 준수`, 'success');
        }
      });
      
      // 교사별 과목 시수 결과 표시
      addLog('📚 교사별 과목 시수 결과:', 'info');
      ...
      teachers.forEach(teacher => {
        ...
        if (teacherSubjects.length > 0) {
          ...
          teacherSubjects.forEach(subjectName => {
            const currentHours = teacherHours[teacher.name]?.subjects[subjectName] || 0;
            const requiredHours = teacher.subjectHours?.[subjectName] || 0;
            ...
            if (requiredHours > 0) {
              ...
              addLog(`  ℹ️ ${subjectName}: ${currentHours}시간`, 'info');
            }
          });
        }
      });
      ...
      // 학급별 과목 시수 결과 표시
      addLog('📚 학급별 과목 시수 결과:', 'info');
      
      classNames.forEach(className => {
        const classSubjectHours = getClassSubjectHours(schedule, className);
        const subjects = data.subjects || [];
        
        addLog(`${className} 과목별 시수:`, 'info');
        
        subjects.forEach(subject => {
          const currentHours = classSubjectHours[subject.name] || 0;
          const targetHours = subject.weekly_hours || 0;
          
          if (targetHours > 0) {
            const balance = currentHours - targetHours;
            
            if (balance === 0) {
              addLog(`  ✅ ${subject.name}: ${currentHours}/${targetHours}시간 (균형)`, 'success');
            } else if (balance > 0) {
              addLog(`  ⚠️ ${subject.name}: ${currentHours}/${targetHours}시간 (+${balance}시간 초과)`, 'warning');
            } else {
              addLog(`  ℹ️ ${subject.name}: ${currentHours}/${targetHours}시간 (${Math.abs(balance)}시간 부족)`, 'info');
            }
          } else {
            addLog(`  ℹ️ ${subject.name}: ${currentHours}시간`, 'info');
          }
        });
      });
      ...
      // 수업 시수 통계
      addLog(`📊 수업 시수 통계:`, 'info');
      addLog(`  • 총 배치된 수업: ${placedCount + filledSlots}시간`, 'info');
      addLog(`  • 1차 배치: ${placedCount}시간`, 'info');
      addLog(`  • 빈 슬롯 채움: ${filledSlots}시간`, 'info');
      addLog(`  • 총 슬롯: ${totalSlots}개`, 'info');
      addLog(`  • 빈 슬롯: ${emptySlots}개`, 'info');
      addLog(`  • 채움률: ${fillRate}%`, 'info');
      
      addLog('시간표 생성이 성공적으로 완료되었습니다!', 'success');
      setGenerationProgress(100);

              // 9단계: 제약조건 검증
        addLog('9단계: 제약조건 검증을 시작합니다.', 'info');
        validateDailySubjectOnceConstraints(schedule, addLog);
        validateCoTeachingConstraints(schedule, addLog);
        validateTeacherConstraints(schedule, addLog);
        validateClassHoursConstraints(schedule, addLog);
          ...
          subjects.forEach(subject => {
            ...
            if (remainingHours > 0) {
              ...
              if (availableTeachers.length === 0) {
                ...
                availableTeachers = secondaryTeachers;
                if (availableTeachers.length > 0) {
                  addLog(`  📝 자동생성 ${className} ${subject.name}: 2단계 교사 배정 (${availableTeachers.length}명)`, 'info');
                }
              }
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">{stats.fillRate}%</div>
