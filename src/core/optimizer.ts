...
class Optimizer {
  optimize(
    ...
    let currentScore = this.calculateScore(currentEntries, subjects, teachers);
    let improved = true;
    let iterations = 0;
    ...
    while (improved && iterations < maxIterations) {
      ...
      for (let i = 0; i < currentEntries.length; i++) {
        for (let j = i + 1; j < currentEntries.length; j++) {
          ...
          const validator = new ConstraintValidator(swapped, subjects, teachers);
  ...
  private calculateScore(
    ...
    for (const softConstraint of softConstraints) {
      const value = softConstraint.evaluate(entries, subjects, teachers);
