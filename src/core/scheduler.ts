  ...
  classNames.forEach(className => {
    subjects.forEach(subject => {
      ...
      if (remainingHours > 0) {
        ...
        // 블록제 과목인지 확인 (subject.block === true)
        if (subject.block === true) {
          addLog(`📋 ${subject.name}은 블록제 과목으로 설정되어 랜덤 배치에서 제외됩니다. (전용 블록제 배치 단계에서 처리)`, 'info');
          return; // 이 과목은 블록제 전용 배치 단계에서만 처리
        }
...
export const executePlacementPlan = (
  ...
  while (finalPlacementPlan.length > 0 && attempts < maxAttempts) {
    ...
    const teachersWithPriority = placement.availableTeachers.map(teacher => {
      ...
      const currentClassHours = getCurrentTeacherHours(schedule, teacher.name, placement.className, data);
...
export const executePlacementPlanStrict = (
  ...
  const maxAttempts = placementPlan.length * 200; // 더 많은 시도 허용
  let attempts = 0;
  const failedPlacements: PlacementPlan[] = [];
  const constraintViolations: string[] = [];

  // 블록제 과목이 있는 수업과 일반 수업 분리
  const blockPlacements = placementPlan.filter(p => {
    const subject = data.subjects?.find(s => s.name === p.subject);
    return subject?.block === true;
  });
  const regularPlacements = placementPlan.filter(p => {
    const subject = data.subjects?.find(s => s.name === p.subject);
  ...
  // 블록제 과목이 있는 수업 우선 정렬 (높은 우선순위부터)
  blockPlacements.sort((a, b) => b.priority - a.priority);
  regularPlacements.sort((a, b) => b.priority - a.priority);

  // 블록제 과목이 있는 수업을 먼저 배치한 후 일반 수업 배치
  const finalPlacementPlan = [...blockPlacements, ...regularPlacements];
  
  addLog(`📋 엄격 배치 계획: 블록제 과목 수업 ${blockPlacements.length}개, 일반 수업 ${regularPlacements.length}개`, 'info');

  while (finalPlacementPlan.length > 0 && attempts < maxAttempts) {
    attempts++;
    const placement = finalPlacementPlan.shift()!;

    // 교사별 시수 균형을 고려하여 교사 선택 (지능적 시수 제한 적용)
    const teachersWithPriority = placement.availableTeachers.map(teacher => {
      const currentHours = teacherHours[teacher.name]?.current || 0;
      const maxHours = teacher.max_hours_per_week || teacherHours[teacher.name]?.max || teacher.maxHours || 22;
      const remainingHours = maxHours - currentHours;
      ...
      const subject = data.subjects?.find(s => s.name === placement.subject);
      const isBlockPeriod = subject?.block === true;
      const requiredHours = isBlockPeriod ? 2 : 1;

      // 🚨 시수 초과 교사는 절대 배제 (엄격 적용)
      if (remainingHours < requiredHours) {
        constraintViolations.push(`${teacher.name} 교사 시수 초과: 현재 ${currentHours}시간, 최대 ${maxHours}시간, 필요 ${requiredHours}시간`);
        return { teacher, priority: -1, remainingHours, excluded: true, reason: 'hours_exceeded' };
      }

      // 🚨 해당 학급 시수 제한 엄격 확인 (교과과목만)
      const classKey = convertClassNameToKey(placement.className);
      const classHoursLimit = teacher.weeklyHoursByGrade?.[classKey] || 0;
      const currentClassHours = getCurrentTeacherHours(schedule, teacher.name, placement.className, data);
      
      // 창의적 체험활동은 학급별 시수 제한에서 제외
      const isCreativeActivity = subject?.category === '창의적 체험활동';
      
      if (classHoursLimit > 0 && !isCreativeActivity && currentClassHours >= classHoursLimit) {
        constraintViolations.push(`${teacher.name} 교사 ${placement.className} 학급 시수 제한 초과: 현재 ${currentClassHours}시간, 제한 ${classHoursLimit}시간`);
        return { teacher, priority: -1, remainingHours, excluded: true, reason: 'class_hours_exceeded' };
      }
      ...
      // 교사 시수 부족도 (부족할수록 우선) - 시수 제한을 지능적으로 적용
      priority += Math.max(0, maxHours - remainingHours) * 100;
    ...
    if (availableTeachers.length === 0) {
      addLog(`🚨 ${placement.subject} 과목을 가르칠 수 있는 시수 여유가 있는 교사가 없습니다.`, 'error');
      addLog(`제약조건 위반: ${constraintViolations.join(', ')}`, 'error');
      failedPlacements.push(placement);
      constraintViolations.length = 0; // 로그 후 초기화
      continue;
    }
    ...
    for (const teacherInfo of availableTeachers) {
      ...
      if (isExplicitlyForbidden) {
        constraintViolations.push(`${teacher.name} 교사는 ${placement.className}에서 수업할 수 없도록 명시적으로 제한됨`);
        continue;
      }
      ...
      if (hasExplicitAssignment) {
        ...
        const maxHours = teacherHours[teacher.name]?.max || 25;
        if (currentHours >= maxHours) {
          constraintViolations.push(`${teacher.name} 교사 시수 제한 초과: ${currentHours}/${maxHours}시간`);
          continue;
        }
      }

      // 🚨 일반 모드로 먼저 시도 (응급 모드 금지)
      let availableSlots = findAvailableSlots(schedule, placement.className, teacher, placement.subject, data);
      ...
      if (availableSlots.length > 0) {
        ...
        const finalConflictCheck = checkTeacherTimeConflict(schedule, teacher.name, selectedSlot.day, selectedSlot.period, placement.className);
        if (!finalConflictCheck.allowed) {
          constraintViolations.push(`교사 중복 배정: ${teacher.name} - ${selectedSlot.day}요일 ${selectedSlot.period}교시`);
          continue; // 절대 배치하지 않음
        }
        
        // 🚨 배치 직전 수업 불가 시간 최종 확인 (절대 우회 금지)
        const unavailableCheck = checkTeacherUnavailable(teacher, selectedSlot.day, selectedSlot.period);
        if (!unavailableCheck.allowed) {
          constraintViolations.push(`교사 불가능 시간 위반: ${teacher.name} - ${selectedSlot.day}요일 ${selectedSlot.period}교시`);
          continue;
        }

        // 🚨 슬롯 배치 전 최종 중복 검증
        if (!validateSlotPlacement(schedule, placement.className, selectedSlot.day, selectedSlot.period, teacher, placement.subject, data, addLog)) {
          constraintViolations.push(`슬롯 배치 검증 실패: ${placement.className} ${selectedSlot.day}요일 ${selectedSlot.period}교시`);
          continue;
        }
        ...
        if (isBlockPeriod) {
          ...
          if (blockPlaced) {
            ...
            constraintViolations.push(`블록제 수업 배치 실패: ${placement.className} ${placement.subject} (${teacher.name}) - ${selectedSlot.day}요일 ${selectedSlot.period}교시`);
            continue; // 블록제 배치 실패 시 다음 교사 시도
          }
        } else {
          // 일반 수업 배치
    ...
    if (!placed) {
      ...
      if (constraintViolations.length > 0) {
        addLog(`🚨 ${placement.subject} 배치 실패 - 제약조건 위반:`, 'error');
  ...
  if (failedPlacements.length > 0) {
    addLog(`🚨 총 ${failedPlacements.length}개의 수업 배치 실패`, 'error');
    addLog('제약조건이 너무 엄격하거나 교사/과목 설정을 재검토해주세요.', 'error');
  }

  return placedCount;
};

// 엄격한 빈 슬롯 채우기 (제약조건 엄격 적용)
const fillEmptySlotsStrict = (
    ...
    DAYS.forEach(day => {
      ...
      for (let period = 0; period < maxPeriods; period++) {
        ...
        if (slot === '' || slot === undefined) {
          // 가장 적합한 교사-과목 조합 찾기 (엄격한 제약조건 적용)
          ...
          teachers.forEach(teacher => {
            ...
            // 🚨 시수 여유가 있고 불가능 시간이 아닌 교사만 (엄격 적용)
            if (currentHours < maxHours &&
          ...
          if (bestMatch && 'score' in bestMatch && bestMatch.score > 0) {
            ...
            // 🚨 최종 제약조건 검증 (엄격)
            const finalValidation = validateSlotPlacement(
              schedule, className, day, period, selectedTeacher, selectedSubject.name, data, addLog
            );
            ...
            if (finalValidation) {
              ...
              filled++;
              addLog(`📝 빈 슬롯 채우기 (엄격): ${className} ${day} ${period + 1}교시 ${selectedSubject.name} (${selectedTeacher.name})`, 'info');
            } else {
              constraintViolations.push(`빈 슬롯 채우기 실패: ${className} ${day} ${period + 1}교시 - 제약조건 위반`);
            }
          }
        }
      }
  ...
  if (constraintViolations.length > 0) {
    addLog(`🚨 빈 슬롯 채우기 중 제약조건 위반:`, 'warning');
    constraintViolations.forEach(violation => {
      addLog(`   - ${violation}`, 'warning');
    });
  }
...
const validateTeacherHoursStrict = (
  ...
  addLog('🔍 교사 시수 제한을 엄격하게 검증합니다.', 'info');
  ...
  if (isValid) {
    addLog('✅ 모든 교사의 시수 제한이 엄격하게 준수되었습니다.', 'success');
  } else {
    addLog(`❌ ${violations.length}건의 시수 제한 위반이 발견되었습니다.`, 'error');
  }
  
  return { isValid, violations };
};

// 최종 제약조건 검증 (모든 제약조건 엄격 적용)
...
// 블록제 수업을 먼저 배치하는 함수 (랜덤 배치 전)
const placeBlockPeriodSubjectsFirst = (
  schedule: Schedule,
  data: TimetableData,
  teacherHours: TeacherHoursTracker,
  addLog: (message: string, type?: string) => void
  ...
  // 블록제 교사 제약조건이 있는 교사들 찾기 (최우선)
  const blockPeriodConstraints = data.constraints?.must?.filter(c => c.type === 'block_period_requirement') || [];
  const blockPeriodTeachers = blockPeriodConstraints.map(c => c.subject);
  
  // 블록제 과목 찾기 (subject.block === true인 과목들)
  const blockPeriodSubjects = subjects.filter(subject => subject.block === true);
  
  if (blockPeriodTeachers.length === 0 && blockPeriodSubjects.length === 0) {
    addLog('📋 블록제 과목이나 블록제 교사가 설정되지 않았습니다.', 'info');
    return 0;
  }
  
  addLog(`📋 블록제 교사 ${blockPeriodTeachers.length}명 발견: ${blockPeriodTeachers.join(', ')}`, 'info');
  addLog(`📋 블록제 과목 ${blockPeriodSubjects.length}개 발견: ${blockPeriodSubjects.map(s => s.name).join(', ')}`, 'info');
  
  // 각 블록제 과목에 대해 배치 계획 수립
  const blockPeriodPlacements: Array<{
    teacherName: string;
    teacher: Teacher;
    subjectName: string;
    className: string;
    priority: number;
    isBlockPeriodTeacher: boolean;
  }> = [];
      ...
      classNames.forEach(className => {
        ...
        if (classSchedule) {
          for (const day of Object.keys(classSchedule)) {
            ...
            if (daySchedule) {
              for (let period = 0; period < Object.keys(daySchedule).length; period++) {
                const slot = daySchedule[period];
        ...
        // 배치해야 할 시수 계산 (블록제는 2시간씩 배치)
        const remainingHours = weeklyHours - currentHours;
        const blockPeriodsNeeded = Math.floor(remainingHours / 2);
        
        for (let i = 0; i < blockPeriodsNeeded; i++) {
          // 교사 시수 제한 확인
          const teacherCurrentHours = teacherHours[teacherName]?.current || 0;
          const teacherMaxHours = teacher.max_hours_per_week || teacherHours[teacherName]?.max || 22;
          
          if (teacherCurrentHours + 2 > teacherMaxHours) {
            return; // 교사 시수 제한 초과
          }
          ...
          const classKey = convertClassNameToKey(className);
          const classHoursLimit = teacher.weeklyHoursByGrade?.[classKey] || 0;
          const currentClassHours = getCurrentTeacherHours(schedule, teacherName, className, data);
          
          if (classHoursLimit > 0 && currentClassHours + 2 > classHoursLimit) {
            return; // 학급 시수 제한 초과
          }
          
          // 최우선순위 (블록제 교사)
          let priority = 10000; // 가장 높은 우선순위
          
          // 교사 시수 부족도 (부족할수록 우선)
          const teacherRemainingHours = teacherMaxHours - teacherCurrentHours;
          priority += Math.max(0, teacherMaxHours - teacherRemainingHours) * 100;
          
          // 랜덤 요소
          priority += Math.random() * 10;
          
          blockPeriodPlacements.push({
            teacherName: teacher.name,
            teacher,
            subjectName,
            className,
            priority,
            isBlockPeriodTeacher: true
          });
        }
      });
    });
  ...
  // 2단계: 블록제 과목(subject.block === true) 배치
  blockPeriodSubjects.forEach(subject => {
    const subjectName = subject.name;
    
    // 해당 과목을 가르칠 수 있는 교사들 찾기
    const availableTeachers = teachers.filter(teacher => 
      teacher.subjects && teacher.subjects.includes(subjectName)
    );
    
    if (availableTeachers.length === 0) {
      addLog(`⚠️ 블록제 과목 ${subjectName}을(를) 가르칠 수 있는 교사가 없습니다.`, 'warning');
      return;
    }
    ...
    classNames.forEach(className => {
      ...
      for (let i = 0; i < blockPeriodsNeeded; i++) {
        ...
        availableTeachers.forEach(teacher => {
          ...
          const teacherMaxHours = teacher.max_hours_per_week || teacherHours[teacher.name]?.max || 22;
  ...
  // 우선순위에 따라 정렬
  blockPeriodPlacements.sort((a, b) => b.priority - a.priority);
  
  addLog(`📋 블록제 수업 배치 계획: ${blockPeriodPlacements.length}개 블록`, 'info');
  
  // 블록제 수업 배치 시도
  for (const placement of blockPeriodPlacements) {
    const { teacherName, teacher, subjectName, className, isBlockPeriodTeacher } = placement;
    ...
    if (teacherCurrentHours + 2 > teacherMaxHours) {
      addLog(`⚠️ ${teacherName} 교사 시수 제한으로 블록제 수업 배치 불가: ${teacherCurrentHours + 2}/${teacherMaxHours}시간`, 'warning');
      continue;
    }
    ...
    if (classHoursLimit > 0 && currentClassHours + 2 > classHoursLimit) {
      addLog(`⚠️ ${teacherName} 교사 ${className} 학급 시수 제한으로 블록제 수업 배치 불가: ${currentClassHours + 2}/${classHoursLimit}시간`, 'warning');
      continue;
    }
    
    // 적합한 슬롯 찾기
    let placed = false;
    const days = Object.keys(schedule[className] || {});
    ...
    for (const day of days) {
      ...
      // 홀수 교시만 확인 (1, 3, 5, 7교시)
      for (let period = 1; period <= maxPeriods - 1; period += 2) {
        if (placed) break;
        
        // 현재 교시와 다음 교시가 모두 비어있는지 확인
        const slotIndex = period - 1;
        const nextSlotIndex = period;
        const currentSlot = schedule[className]?.[day]?.[slotIndex];
        const nextSlot = schedule[className]?.[day]?.[nextSlotIndex];
        ...
        if (currentSlot && typeof currentSlot === 'object' && currentSlot.subject) {
          continue; // 현재 교시가 이미 사용 중
        }
        
        if (nextSlot && typeof nextSlot === 'object' && nextSlot.subject) {
          continue; // 다음 교시가 이미 사용 중
        }
        
        // 교사가 다음 교시에 다른 학급에서 수업하는지 확인
        ...
        if (!teacherConflictNext.allowed) {
          continue; // 교사가 다음 교시에 다른 학급에서 수업 중
        }
        
        // 교사 불가능 시간 확인
        const unavailableCheck = checkTeacherUnavailable(teacher, day, period);
        if (!unavailableCheck.allowed) {
          continue; // 교사가 현재 교시에 수업할 수 없음
        }
        
        const unavailableCheckNext = checkTeacherUnavailable(teacher, day, period + 1);
        if (!unavailableCheckNext.allowed) {
          continue; // 교사가 다음 교시에 수업할 수 없음
        }
        
        // 블록제 수업 배치 시도
        const success = placeBlockPeriodSubject(schedule, className, day, period, teacherName, teacher, data, subjectName);
        
        if (success && subjectName) {
          // 교사 시수 업데이트
          teacherHours[teacherName].current += 2;
          teacherHours[teacherName].subjects[subjectName] = (teacherHours[teacherName].subjects[subjectName] || 0) + 2;
          
          const teacherType = isBlockPeriodTeacher ? '블록제 교사' : '블록제 과목';
          addLog(`✅ 블록제 수업 배치: ${teacherName} 교사(${teacherType}) ${className} ${day} ${period}-${period + 1}교시 ${subjectName}`, 'success');
          placedCount++;
          placed = true;
        }
      }
    ...
    if (!placed) {
      const teacherType = isBlockPeriodTeacher ? '블록제 교사' : '블록제 과목';
      addLog(`⚠️ ${teacherName} 교사(${teacherType}) ${className} ${subjectName} 블록제 수업 배치 실패: 적합한 슬롯 없음`, 'warning');
    }
  }
  
  addLog(`📊 블록제 수업 배치 완료: ${placedCount}개 블록 배치됨`, 'info');
  return placedCount;
};
...
export const generateTimetable = async (
  ...
  if (!hasErrors) {
    addLog('4단계: 공동수업을 엄격한 제약조건 하에 배치합니다.', 'info');
  ...
  // 5단계: 블록제 수업 배치 (세 번째 우선순위 - 랜덤 배치 전)
  if (!hasErrors) {
    addLog('5단계: 블록제 수업을 먼저 배치합니다. (랜덤 배치 전)', 'info');
    blockPeriodCount = placeBlockPeriodSubjectsFirst(schedule, data, teacherHours, addLog);
    addLog(`✅ 블록제 수업 ${blockPeriodCount}개 배치 완료`, 'success');
    setProgress?.(50);
  }

  // 6단계: 나머지 수업 배치 계획 수립 (엄격한 제약조건 준수)
  if (!hasErrors) {
    addLog('6단계: 나머지 수업 배치 계획을 수립합니다. (모든 제약조건 엄격 적용)', 'info');
    const placementPlan = createPlacementPlan(schedule, data, addLog);
    addLog(`총 ${placementPlan.length}개의 수업을 엄격한 제약조건을 지켜서 배치합니다.`, 'info');
    setProgress?.(55);

    // 7단계: 나머지 수업 엄격 배치 (제약조건 절대 위반 금지)
    addLog('7단계: 나머지 수업을 엄격한 제약조건을 지켜서 배치합니다. (절대 위반 금지)', 'info');
    placedCount = executePlacementPlanStrict(schedule, placementPlan, data, teacherHours, addLog);
  ...
  // 8단계: 교사 시수 제한 엄격 검증
  if (!hasErrors) {
    addLog('8단계: 교사 시수 제한을 엄격하게 검증합니다.', 'info');
    const teacherValidationResult = validateTeacherHoursStrict(schedule, data, teacherHours, addLog);
    if (!teacherValidationResult.isValid) {
      addLog('🚨 교사 시수 제한 위반이 발견되었습니다!', 'error');
      addLog('⚠️ 오류가 발생했지만 부분적으로 생성된 시간표를 확인할 수 있습니다.', 'warning');
      hasErrors = true;
      errorMessage = '교사 시수 제한 위반으로 인한 시간표 생성 실패';
    }
    setProgress?.(65);
  }

  // 9단계: 빈 슬롯 채우기 (제약조건 엄격 적용)
  if (!hasErrors) {
    addLog('9단계: 빈 슬롯을 제약조건을 엄격하게 지켜서 채웁니다.', 'info');
    filledSlots = fillEmptySlotsStrict(schedule, data, teacherHours, addLog);
    setProgress?.(70);
  }

  // 10단계: 최종 제약조건 검증 (절대 필수!)
  if (!hasErrors) {
    addLog('10단계: 모든 제약조건을 최종 검증합니다.', 'info');
    const finalValidationResult = performFinalValidation(schedule, data, teacherHours, addLog);
    if (!finalValidationResult.isValid) {
      addLog('🚨 최종 제약조건 검증에서 위반이 발견되었습니다!', 'error');
      addLog('⚠️ 오류가 발생했지만 부분적으로 생성된 시간표를 확인할 수 있습니다.', 'warning');
      hasErrors = true;
      errorMessage = '최종 제약조건 검증 실패';
    }
    setProgress?.(75);
  }

  // 11단계: 통계 계산
  addLog('11단계: 통계를 계산합니다.', 'info');
  ...
  if (hasErrors) {
    ...
    addLog(`🎉 시간표 생성 완료: ${placedCount + filledSlots + blockPeriodCount + coTeachingCount}개 수업 배치 (모든 제약조건 준수)`, 'success');
  }
  setProgress?.(100);
