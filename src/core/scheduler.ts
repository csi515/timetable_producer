...
export class Scheduler {
  ...
  generate(): ScheduleResult {
    ...
    entries = optimizer.optimize(entries, this.subjects, this.teachers, this.config);
    
    // 4. 최종 검증
    const validator = new ConstraintValidator(entries, this.subjects, this.teachers);
  ...
  generateMultiple(minCount: number = 3, maxAttempts: number = 50): MultipleScheduleResult {
    ...
    while (results.length < minCount && generationAttempts < maxAttempts) {
      ...
      // 최적화
      entries = optimizer.optimize(entries, currentSubjects, currentTeachers, this.config);
      
      // 검증
      const validator = new ConstraintValidator(entries, currentSubjects, currentTeachers);
      const violations = validator.validateAll();
      const score = validator.calculateScore();
