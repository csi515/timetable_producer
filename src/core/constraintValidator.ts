...
export class ConstraintValidator {
  ...
  constructor(entries: TimetableEntry[], subjects: Subject[], teachers: Teacher[]) {
  ...
  validateAll(): ConstraintViolation[] {
    ...
    for (const entry of this.entries) {
      for (const constraint of criticalConstraints) {
        if (!constraint.check(entry, this.entries, this.subjects, this.teachers)) {
          violations.push({
  ...
  calculateScore(): number {
    ...
    for (const softConstraint of softConstraints) {
      const value = softConstraint.evaluate(this.entries, this.subjects, this.teachers);
